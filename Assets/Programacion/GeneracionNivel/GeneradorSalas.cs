...
{
    [SerializeField] NivelDef salasNivel;
    ...
    private void colocarSalas()
    {
        ...
        HabitacionRenderer renderer = GetComponent<HabitacionRenderer>();

        for (int i = 0; i < casillasOcupadas.Count; i++)
        {
            Habitacion habitacion = new Habitacion(casillasOcupadas[i].x1, casillasOcupadas[i].x2, casillasOcupadas[i].y1, casillasOcupadas[i].y2);
            renderer.InstantiateRoom(habitacion, casillasOcupadas[i].getCentro());
